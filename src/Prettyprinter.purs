...
import Data.String.Regex.Flags  as String.Regex
...
-- 'Functor'-polymorphic values), but @'map'@ is much less readable compared to
...
nest
    :: forall ann . Int -- ^ Change of nesting level
    -> Doc ann
    -> Doc ann
...
-- @
hang
    :: forall ann . Int -- ^ Change of nesting level, relative to the start of the first line
    -> Doc ann
    -> Doc ann
...
-- @
indent
    :: forall ann . Int -- ^ Number of spaces to increase indentation by
    -> Doc ann
    -> Doc ann
...
-- 'punctuate'.
encloseSep
    :: forall ann . Doc ann   -- ^ left delimiter
...
encloseSep l r s ds = case ds of
    []  -> l <> r
    [d] -> l <> d <> r
    _   -> cat (Array.zipWith (<>) (Array.cons l (Array.replicate (Array.length ds - 1) s)) ds) <> r
...
concatWithSpace x y = x <> Text 1 " " <> y

infixr 5 concatWithSpace as <+> -- like <>
...
concatWith :: forall ann f . Container f => Foldable f => (Doc ann -> Doc ann -> Doc ann) -> f (Doc ann) -> Doc ann
concatWith f =
  Container.uncons
    >>> case _ of
        Nothing -> Empty
...
--     fillSep :: [Doc] -> Doc
fill
    :: forall ann . Int -- ^ Append spaces until the document is at least this wide
    -> Doc ann
    -> Doc ann
...
--
-- >>> case spaces 1 of Text 1 " " -> true; _ -> false
-- true
--
...
removeTrailingWhitespace = go (RecordedWhitespace [] 0)
  where
    commitWhitespace
        :: forall ann
         . Array Int -- Withheld lines
        -> Int -- Withheld spaces
        -> SimpleDocStream ann
        -> SimpleDocStream ann
    commitWhitespace is n sds = case Array.uncons is of
        Nothing -> case n of
                       0 -> sds
                       1 -> SText 1 " " sds
                       _ -> SText n (textSpaces n) sds
        Just { head: i, tail: is' } -> let end = SLine (i + n) sds
                   in prependEmptyLines is' end

    prependEmptyLines :: forall ann . Array Int -> SimpleDocStream ann -> SimpleDocStream ann
    prependEmptyLines is sds0 = foldr (\_ sds -> SLine 0 sds) sds0 is

    go :: forall ann . WhitespaceStrippingState -> SimpleDocStream ann -> SimpleDocStream ann
    ...
    go (RecordedWhitespace withheldLines withheldSpaces) = \sds -> case sds of
        ...
        SText textLength text rest ->
            let stripped :: String
                stripped = String.Regex.replace (String.Regex.unsafeRegex "\\s$" String.Regex.global) "" text

                strippedLength = String.length stripped
        ...
        SLine i rest -> go (RecordedWhitespace (Array.cons i withheldLines) 0) rest
        SAnnPush ann rest -> commitWhitespace
                                 withheldLines
                                 withheldSpaces
                                 (SAnnPush ann (go (AnnotationLevel 1) rest))
        SAnnPop _ -> unsafeCrashWith panicSAnnPop
...
-- >>> go = String.putStrLn <<< renderStrict <<< removeTrailingWhitespace <<< layoutPretty defaultLayoutOptions
-- >>> go doc
-- lorem
-- <BLANKLINE>
-- ipsum
...
derive instance functorSimpleDocStream :: Functor SimpleDocStream
  ...
  foldMap f = go
    where
      go = \sds -> case sds of
          ...
          SAnnPush ann rest -> f ann `append` go rest
          SAnnPop rest      -> go rest
...
data PageWidth
    ...
    --
    --   - The 'Number' is the ribbon with, i.e. the fraction of the total
...
remainingWidth :: forall ann . Int -> Number -> Int -> Int -> Int
  ...
  where
    ...
    ribbonWidth =
        (max 0 <<< min lineLength <<< floor)
            (Int.toNumber lineLength * ribbonFraction)
...
defaultLayoutOptions = { layoutPageWidth: defaultPageWidth }
...
-- >>> let go layouter x = (String.putStrLn <<< renderStrict <<< layouter (LayoutOptions (AvailablePerLine 26 1))) (vsep [hr, x, hr])
--
...
layoutSmart
    :: forall ann . LayoutOptions
    -> Doc ann
    -> SimpleDocStream ann
layoutSmart { layoutPageWidth: pageWidth_@(AvailablePerLine lineLength ribbonFraction) } =
  ...
  where
    ...
    fits :: forall ann . Int -> Int -> Maybe Int -> SimpleDocStream ann -> Boolean
...
layoutSmart { layoutPageWidth: Unbounded } = layoutUnbounded
...
layoutUnbounded =
  ...
  where
    ...
    failsOnFirstLine :: forall ann . SimpleDocStream ann -> Boolean
    failsOnFirstLine = go
      where
        go sds = case sds of
            SFail        -> true
...
layoutWadlerLeijen
  ...
  where
    ...
    initialIndentation :: forall ann . SimpleDocStream ann -> Maybe Int
...
layoutCompact = scan 0 <<< Array.singleton
  where
    scan col docs =
      case Array.uncons docs of
           Nothing -> SEmpty
           Just { head: d, tail: ds } ->
             case d of
              Fail            -> SFail
              ...
              FlatAlt x _     -> scan col (Array.cons x ds)
              Line            -> SLine 0 (scan 0 ds)
              Cat x y         -> scan col ([x, y] <> ds)
              Nest _ x        -> scan col (Array.cons x ds)
              Union _ y       -> scan col (Array.cons y ds)
...
instance showDoc :: Show (Doc ann) where
    show doc = render (layoutPretty defaultLayoutOptions doc)
...
--     'showsPrec' _ = 'renderShow' <<< 'layoutPretty' 'defaultLayoutOptions' <<< 'pretty'
-- @
render :: forall ann . SimpleDocStream ann -> String
render = \sds -> case sds of
    SFail        -> unsafeCrashWith panicUncaughtFail
    SEmpty       -> ""
    SText _l t x -> t <> render x
    SLine i x    -> "\n" <> textSpaces i <> render x
...
textSpaces n = fold $ Array.replicate n " "
